...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = (function () {
	    ...
	    _createClass(Game, {
	        ...
	        setup_welcome_screen: {
	            value: function setup_welcome_screen() {
	                ...
	                this.welcome_screen = new WelcomeScreen(this.spl, {
	                    on_gamestart: function (player_name) {
	        ...
	        setup_socket: {
	            value: function setup_socket() {
	                ...
	                this.socket.on("player_joined", function (player) {
	                    ...
	                    _this.other_players.push(new OtherWarrior(_this.spl, player.pos, player.name));
	                });
	        ...
	        update_other_player: {
	            value: function update_other_player(player_info) {
	                var player = this.other_players.find(function (player) {
	                    return player_info.name === player.name;
	                });
	                if (player) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SpriteLoader = (function () {
	    ...
	    _createClass(SpriteLoader, {
	        ...
	        draw_on_map: {
	            value: function draw_on_map(sprite, element, mapX, mapY) {
	                var _ref = arguments[4] === undefined ? {} : arguments[4];
	                ...
	                var canvasX = mapX - window.camera[0];
	                var canvasY = mapY - window.camera[1];
	                var meta = this.get_meta(sprite, element, frame);
	                this.ctx.drawImage(this[sprite], meta.x, meta.y, meta.width, meta.height, canvasX * unit, canvasY * unit, meta.width * scale, meta.height * scale);
	                return meta.next_frame;
	            }
	    ...
	    _createClass(World, {
	        draw: {
	            value: function draw(char_pos) {
	                if (char_pos) {
	                    var x = char_pos[0] - window.camera[0];
	                    var y = char_pos[1] - window.camera[1];
	    ...
	    _createClass(Warrior, {
	        ...
	        draw_name: {
	            value: function draw_name() {
	                var name = this.name;
	                // clean this up later
	                var canvas_x = this.pos[0] - window.camera[0];
	                var canvas_y = this.pos[1] - window.camera[1];
	    ...
	    _createClass(Character, {
	        ...
	        draw: {
	            value: function draw() {
	                ...
	                var _get_floor_ceil = this.get_floor_ceil();
	
	                var ceil_x = _get_floor_ceil.ceil_x;
	                var ceil_y = _get_floor_ceil.ceil_y;
	                var floor_x = _get_floor_ceil.floor_x;
	                var floor_y = _get_floor_ceil.floor_y;
	        ...
	        get_floor_ceil: {
	            value: function get_floor_ceil() {
	                var ceil_x = Math.ceil(this.pos[0]);
	                var ceil_y = Math.ceil(this.pos[1]);
	
	                var floor_x = Math.floor(this.pos[0]);
	                var floor_y = Math.floor(this.pos[1]);
	
	                return { ceil_x: ceil_x, ceil_y: ceil_y, floor_x: floor_x, floor_y: floor_y };
	            }
	        }
	    });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var OtherWarrior = (function (_Warrior) {
	    ...
	    _inherits(OtherWarrior, _Warrior);
	
	    _createClass(OtherWarrior, {
	        update_values: {
	            value: function update_values(pos, action, next_frame) {
	        ...
	        draw: {
	            value: function draw() {
	                ...
	                this.keyboard.on_keyup_space = function () {
	                    ...
	                    _this.on_gamestart(player_name);
	                };
